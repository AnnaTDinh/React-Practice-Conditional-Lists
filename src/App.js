...
class App extends Component {
  ...
  handleValue = (event) => {
    ...
    let userLength = event.target.value.length;
    this.setState({ userInput: userInput, length: userLength });
  };
  deleteCharHandler = (i) => {
    let input = this.state.userInput.split('');
    input.splice(i, 1);
    let deleted = input.join('');
    this.setState({
      userInput: deleted,
    })
  }
  render() {
    const charList = this.state.userInput.split('').map((char, i) => {
      return <CharComponents clicked={() => this.deleteCharHandler(i)} character={char} key={i}/>
    })
    return (
      <div className="App">
        <input
